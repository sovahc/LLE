...
namespace LLE
{
	public class MyGridIntersection
	{
		...
		static Vector3I GetGridPoint(ref Vector3D v, Vector3I min, Vector3I max)
		{
			...
			else if (v.X >= max.X + 1) { v.X = NextAfter(max.X + 1, float.NegativeInfinity); r.X = max.X; }
			else r.X = (int)Math.Floor(v.X);

			if (v.Y < min.Y) { v.Y = r.Y = min.Y; }
			else if (v.Y >= max.Y + 1) { v.Y = NextAfter(max.Y + 1, float.NegativeInfinity); r.Y = max.Y; }
			else r.Y = (int)Math.Floor(v.Y);
			
			if (v.Z < min.Z) { v.Z = r.Z = min.Z; }
			else if (v.Z >= max.Z + 1) { v.Z = NextAfter(max.Z + 1, float.NegativeInfinity); r.Z = max.Z; }
		...
		// s.o.v.a
		public static IMySlimBlock FastRaycast(MyCubeGrid grid, Vector3D worldStart, Vector3D worldEnd)
		{
			MatrixD invWorld = grid.PositionComp.WorldMatrixNormalizedInv;//MatrixD.Invert(WorldMatrix);
			Vector3D lineStart, lineEnd;
			Vector3D.Transform(ref worldStart, ref invWorld, out lineStart);
			Vector3D.Transform(ref worldEnd, ref invWorld, out lineEnd);

			//We need move the line, because MyGridIntersection calculates the center of the box in the corner
			var offset = grid.GridSizeHalfVector;
			lineStart += offset;
			lineEnd += offset;
			...
			Vector3D start = lineStart / grid.GridSize;
			Vector3D end = lineEnd / grid.GridSize;
			
			if (ClipLine(ref start, ref end, min, max) == false) return null;
			...
			while (true)
			{
				var check = cur * signInt;

				IMySlimBlock slim = grid.GetCubeBlock(check);
