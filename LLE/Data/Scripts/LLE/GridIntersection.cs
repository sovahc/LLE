...
{
	/// <summary>
	/// 3D DDA (Amanatides-Woo) grid traversal, copied from
	/// Sandbox.Engine.Utils.MyGridIntersection in Space Engineers sources.
	/// Cells have their centers in the corners.
	/// </summary>
	public static class GridIntersection
	{
		...
		// Returns the largest double strictly less than x.
		// Replacement for BulletXNA.MathUtil.NextAfter(x, float.NegativeInfinity)
		// which is not whitelisted for mods.
		static double NextBefore(double x)
		{
		...
		static Vector3I GetGridPoint(ref Vector3D v, Vector3I min, Vector3I max)
		{
			...
			else if (v.X >= max.X + 1) { v.X = NextBefore(max.X + 1); r.X = max.X; }
			else r.X = (int)Math.Floor(v.X);

			if (v.Y < min.Y) { v.Y = r.Y = min.Y; }
			else if (v.Y >= max.Y + 1) { v.Y = NextBefore(max.Y + 1); r.Y = max.Y; }
			else r.Y = (int)Math.Floor(v.Y);

			if (v.Z < min.Z) { v.Z = r.Z = min.Z; }
			else if (v.Z >= max.Z + 1) { v.Z = NextBefore(max.Z + 1); r.Z = max.Z; }
		...
		public static void Calculate(List<Vector3I> result, float gridSize, Vector3D lineStart, Vector3D lineEnd, Vector3I min, Vector3I max)
		{
			...
			//BUG - Confirmed by glm-5.2-high: line in plane x=max+1 leaks into grid. Fix: split IntersectionT into strict/non-strict variants.
