...
## MEMORY & CONTINUITY
* **Planning**: a task that spans several turns opens with `plan`. That call is the plan, and it stays in your context for the rest of the task.
* **Plan Maintenance**: plan once at the start and then follow it. Call `plan` again only if the original goal becomes impossible.
* **Persistence**: `memory` survives a context reset, the plan does not. Whatever you must not lose goes into `memory`.
